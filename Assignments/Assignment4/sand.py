...
def check_move(grid, x1, y1, x2, y2):
    ...
    False
    """
    # 感觉这里检查坐下和右下有点漏洞，万一不是就一格而是从0,0到3，3这种多格的话怎么办
    width = len(grid[0])
    height = len(grid)
    ...
    if grid[y2][x2] != None:
        return False
    if y2 > y1 and x2 != x1:
        if grid[y2][x2] != None or grid[y2-1][x2] != None:
            return False
    return True
    ...
    if grid[y][x] != 's':
        ...
        if num < brownian:
            ...
            if coin == 0:
                ...
                if check_move(grid, x, y, x+1, y):
                    return do_move(grid, x, y, x+1, y)
        # 要保证所有路径下都会有东西返回，如果是if条件下写的return会导致有一些边界情况没考虑到没写return，就会直接返回None
        return grid
...
def do_whole_grid(grid, brownian):
    ...
    for y in reversed(range(height)):
        for x in range(width):
            ...
            grid = do_brownian(grid, x, y, brownian)
    return grid
