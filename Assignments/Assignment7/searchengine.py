...
def search(index, query):
    ...
    queries = query.split()
    if len(queries) == 1:
        if queries[0] not in index:
            return []
        else:
            return index[queries[0]]
    if queries[0] not in index:
        return []
    result = index[queries[0]]
    for term in queries[1:]:
        if term not in index:
            f_list = []
        else:
            f_list = index[term]
        result = common(result, f_list)
    return result
